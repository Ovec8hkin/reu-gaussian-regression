...
from trajectories import calculate_trajectory as traj


def regression_trajectories(n_particles=30, integration_time=15, n_timesteps=100, density=None):
    '''
    ...
    '''
    x, y, phi, u, v = spiral_vf.generate_spiral(ratio=0.8)
    ...
    init, _, temp = traj.lagtransport(n_particles, integration_time, n_timesteps, density=density)

    print(temp)

    vels = np.apply_along_axis(traj.get_velocity, 1, temp)
    ...
    Xo = np.concatenate([temp[:, 1][:, None], temp[:, 0][:, None]], axis=1)
    ...
    '''
    Ur, Ku = model_u.predict(GridPoints)  # Kr = posterior covariance
    Vr, Kv = model_v.predict(GridPoints)
    ...
    return x, y, u, v, ur, vr, Xo, ku, kv, model_u, model_v, init


if __name__ == "__main__":
    reg = regression_trajectories(n_particles=30, integration_time=20, n_timesteps=20)
