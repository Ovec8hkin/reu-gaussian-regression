...
class TimeseriesRegression(Regression):
    ...
    def plot_quiver(self, show=True, save=False):
        ...
        plot1.quiver(self.x[::self.ds], self.y[::self.ds], self.u[::self.ds, ::self.ds], self.v[::self.ds, ::self.ds],
                    scale=self.scale)
        plot1.streamplot(self.x, self.y, self.u, self.v)
        plot1.plot(self.Xo[:, self.dim - 1], self.Xo[:, self.dim - 2], 'og', markersize=self.marker_size)
        plot1.set_xlim(-5, 5)
        plot1.set_ylim(-5, 5)
        plot1.set_title('Original Velocity Field (' + str(self.n_drifters) + ' drifters)', size=self.text_size,
                       pad=self.title_pad)
        ...
        plot2.quiver(self.x[::self.ds], self.y[::self.ds], ur[::self.ds, ::self.ds], vr[::self.ds, ::self.ds],
                    scale=self.scale)
        plot2.streamplot(self.x, self.y, ur, vr)
        plot2.plot(xo[:, self.dim - 1], xo[:, self.dim - 2], 'og', markersize=self.marker_size)
        plot2.set_xlim(-5, 5)
        plot2.set_ylim(-5, 5)
        plot2.set_title('GPR Velocity Field (' + str(self.n_drifters) + ' drifters)', size=self.text_size,
                       pad=self.title_pad)
        ...
        axfreq = pl.axes([0.05, 0.15, 0.85, 0.05], facecolor=axcolor)

        sfreq = Slider(axfreq, 'Freq', 0, 10.0, valinit=0, valstep=1)
